...
export default function MgnregsIndividualIssuesListingPage() {
    ...
    const categoriesForType = useMemo(() => {
        ...
        return Array.from(new Set(MOCK_MGNREGS_DATA.filter(d => d.type === filters.type).map(d => d.category)));
    }, [filters.type]);

    const subCategoriesForCategory = useMemo(() => {
        if (filters.category === 'all') return [];
        return Array.from(new Set(MOCK_MGNREGS_DATA.filter(d => d.type === filters.type && d.category === filters.category).map(d => d.subCategory)));
    }, [filters.category, filters.type]);
    
    const codesForSubCategory = useMemo(() => {
        if (filters.subCategory === 'all') return [];
        return MOCK_MGNREGS_DATA.filter(d => d.type === filters.type && d.category === filters.category && d.subCategory === filters.subCategory);
                ...
                <Card>
                    ...
                    <CardContent>
                         <div className="p-4 border rounded-lg bg-card grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 items-end mt-4">
                            ...
                            <Select value={filters.type} onValueChange={(v) => handleFilterChange('type', v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Types</SelectItem>{mgnregsTypes.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select>
