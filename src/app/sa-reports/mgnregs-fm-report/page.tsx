...
const ReportTable = ({ title, data, viewType, loading }: { title: string; data: ReportRow[]; viewType: 'district' | 'block' | 'panchayat', loading: boolean }) => {
    const totalRow = data.reduce((acc, row) => ({
        ...
        raisedIssues: 0, raisedAmount: 0,
        droppedIssues: 0, droppedAmount: 0,
        pendingIssues: 0, pendingAmount: 0
    });
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead colSpan={2} className="text-center">Raised</TableHead>
                <TableHead colSpan={2} className="text-center">Dropped</TableHead>
                <TableHead colSpan={2} className="text-center">Pending</TableHead>
              </TableRow>
              <TableRow>
                <TableHead>No of Issues</TableHead><TableHead>Amount</TableHead>
                <TableHead>No of Issues</TableHead><TableHead>Amount</TableHead>
                <TableHead>No of Issues</TableHead><TableHead>Amount</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
               {loading ? (
                  <TableRow><TableCell colSpan={10} className="h-24 text-center"><Loader2 className="mx-auto animate-spin" /></TableCell></TableRow>
              ) : data.length === 0 ? (
                  <TableRow><TableCell colSpan={10} className="text-center h-24">No issues found.</TableCell></TableRow>
              ) : (
                <>
                  {data.map((row, index) => (
                    <TableRow key={row.name}>
                      ...
                      <TableCell>{row.raisedIssues}</TableCell><TableCell>{row.raisedAmount.toLocaleString()}</TableCell>
                      <TableCell>{row.droppedIssues}</TableCell><TableCell>{row.droppedAmount.toLocaleString()}</TableCell>
                      <TableCell>{row.pendingIssues}</TableCell><TableCell>{row.pendingAmount.toLocaleString()}</TableCell>
                    </TableRow>
                  ))}
                    <TableRow className="font-bold bg-muted/50">
                        ...
                        <TableCell>{totalRow.raisedIssues}</TableCell><TableCell>{totalRow.raisedAmount.toLocaleString()}</TableCell>
                        <TableCell>{totalRow.droppedIssues}</TableCell><TableCell>{totalRow.droppedAmount.toLocaleString()}</TableCell>
                        <TableCell>{totalRow.pendingIssues}</TableCell><TableCell>{totalRow.pendingAmount.toLocaleString()}</TableCell>
                    </TableRow>
                </>
              )}
            </TableBody>
...
export default function MgnregsFmReportPage() {
    ...
    const reportData = useMemo(() => {
        ...
        const fmEntries = yearFilteredEntries.map(entry => ({
            ...entry,
            paraParticulars: entry.paraParticulars?.filter(p => p.type === 'FM - Financial Misappropriation')
        ...
        const processEntries = (geoList: string[], groupBy: 'district' | 'block' | 'panchayat') => {
            ...
            aggregation.forEach((value) => {
                const auditedInGroup = fmEntries.filter(e => {
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <main className="flex-1 container mx-auto px-4 py-8 pb-24 space-y-8">
                <Card>
                    <CardHeader>
                        <CardTitle>R.6.1.B.1 FM - Financial Misappropriation</CardTitle>
                        <CardDescription>A summary of Financial Misappropriation issues for MGNREGS.</CardDescription>
                    </CardHeader>
