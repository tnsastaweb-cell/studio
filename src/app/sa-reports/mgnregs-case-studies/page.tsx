...
export default function MgnregsCaseStudiesPage() {
    ...
    const uniqueBRPs = useMemo(() => {
     ...
     const uniqueCaseStudyNos = useMemo(() => {
        return Array.from(new Set(mgnregsCaseStudies.map(cs => cs.caseStudyNo)));
    }, [mgnregsCaseStudies]);
    
    const handleGetReports = () => {
        let data = [...mgnregsCaseStudies];
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <main className="flex-1 container mx-auto px-4 py-8 pb-24 space-y-8">
                 <Card>
                    <CardHeader>
                        <CardTitle>MGNREGS Case Study Reports</CardTitle>
                        <CardDescription>Filter and view submitted case studies for the MGNREGS scheme.</CardDescription>
                    </CardHeader>
                    <CardContent>
                             ...
                             <Popover open={openBRPPopover} onOpenChange={setOpenBRPPopover}>
                                 ...
                                 <PopoverContent className="w-[300px] p-0">
                                     <Command><CommandInput placeholder="Search BRP..."/><CommandEmpty>No BRP found.</CommandEmpty>
                                         <CommandList>
                                             <CommandItem onSelect={() => setFilters(f => ({...f, brpEmployeeCode: 'all'}))}>All BRPs</CommandItem>
                                             {uniqueBRPs.map((brp) => (
