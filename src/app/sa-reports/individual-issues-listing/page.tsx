    ...
    return (
        <div className="border rounded-lg mt-4 overflow-x-auto">
            <Table>
                ...
                <TableBody>
                    {loading ? (
                         <TableRow><TableCell colSpan={14} className="text-center h-24"><Loader2 className="mx-auto animate-spin" /></TableCell></TableRow>
                    ) : data.length === 0 ? (
                        <TableRow><TableCell colSpan={14} className="text-center h-24">No issues found for the selected filters.</TableCell></TableRow>
                    ) : (
                        data.map((item, index) => (
                            <TableRow key={index}>
                                <TableCell>{index + 1}</TableCell>
                                ...
                                <TableCell>{item.para.recoveryAmount?.toLocaleString()}</TableCell>
                                <TableCell>{item.para.hlcRecoveryAmount?.toLocaleString()}</TableCell>
                                <TableCell>{item.para.paraStatus}</TableCell>
                            </TableRow>
                        ))
...
export default function IndividualIssuesListingPage() {
    ...
    const handleGetReport = () => {
        const sourceData = activeScheme === 'MGNREGS' ? flattenedMgnregs : flattenedPmayg;
    ...
    const categoriesForType = useMemo(() => {
        ...
        const source = activeScheme === 'MGNREGS' ? MOCK_MGNREGS_DATA : MOCK_PMAYG_DATA;
        return Array.from(new Set(source.filter(d => d.type === filters.type).map(d => d.category)));
    }, [filters.type, activeScheme]);
    ...
    }, [filters.category, filters.type, activeScheme]);
    ...
    }, [filters.subCategory, filters.category, filters.type, activeScheme]);
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <main className="flex-1 container mx-auto px-4 py-8 pb-24 space-y-8">
                <Card>
                    <CardHeader>
                        <CardTitle>Individual Issues Listing</CardTitle>
                        <CardDescription>Filter and view individual issues reported across schemes.</CardDescription>
                    </CardHeader>
                    <CardContent>
                         <Tabs defaultValue="MGNREGS" onValueChange={handleSchemeChange} className="w-full">
                            <TabsList>
                                <TabsTrigger value="MGNREGS">R.6.1.A.2 (MGNREGS)</TabsTrigger>
                                <TabsTrigger value="PMAY-G">R.6.2.A.2 (PMAY-G)</TabsTrigger>
                            </TabsList>
                             <div className="p-4 border rounded-lg bg-card grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 items-end mt-4">
                                ...
                                <Select value={filters.type} onValueChange={(v) => handleFilterChange('type', v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Types</SelectItem>{(activeScheme === 'MGNREGS' ? mgnregsTypes : pmaygTypes).map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select>
                            ...
                            <TabsContent value="PMAY-G">
                                {isReportGenerated && <PmaygReportTable data={filteredReportData} loading={pmaygLoading} />}
                            </TabsContent>
                        </Tabs>
                    </CardContent>
                </Card>
