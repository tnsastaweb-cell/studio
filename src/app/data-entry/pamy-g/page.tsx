        ...
        <div className="p-4 border rounded-lg space-y-4 relative bg-slate-50">
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <Controller control={form.control} name={`paraParticulars.${index}.category`} render={({ field }) => (
                     <FormItem className="lg:col-span-2"><FormLabel>Category</FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={!selectedType}><FormControl><SelectTrigger><SelectValue placeholder="Select Category"/></SelectTrigger></FormControl>
             ...
             <div className="space-y-4">
                 <FormField control={form.control} name={`paraParticulars.${index}.subCategory`} render={({ field }) => (
                    <FormItem><FormLabel>Sub Category</FormLabel><FormControl><Textarea readOnly {...field} className="bg-muted min-h-[120px] w-full" /></FormControl></FormItem>
                 )} />
                 ...
                 <FormField control={form.control} name={`paraParticulars.${index}.description`} render={({ field }) => (
                    <FormItem><FormLabel>Description (Max 1000 chars) *</FormLabel><FormControl><Textarea {...field} className="h-28" maxLength={1000} /></FormControl><FormMessage/></FormItem>
                 )} />
             </div>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                 <FormField control={form.control} name={`paraParticulars.${index}.beneficiaries`} render={({ field }) => (<FormItem><FormLabel>No. of Beneficiaries</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage/></FormItem>)} />
                 <FormField control={form.control} name={`paraParticulars.${index}.centralAmount`} render={({ field }) => (<FormItem><FormLabel>Central Amount</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage/></FormItem>)} />
                 <FormField control={form.control} name={`paraParticulars.${index}.stateAmount`} render={({ field }) => (<FormItem><FormLabel>State Amount</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage/></FormItem>)} />
                 <FormField control={form.control} name={`paraParticulars.${index}.otherAmount`} render={({ field }) => (<FormItem><FormLabel>Others Amount</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage/></FormItem>)} />
                 <FormField control={form.control} name={`paraParticulars.${index}.grievances`} render={({ field }) => (<FormItem><FormLabel>No. of Grievances</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage/></FormItem>)} />
...
export default function PmaygDataEntryPage() {
    ...
    const { fields, append, remove } = useFieldArray({
     ...
     const staffList = useMemo(() => {
        const targetRoles: StaffUser['designation'][] = ['BRP', 'DRP', 'DRP I/C', 'ADMIN', 'CREATOR', 'CONSULTANT'];
        return users.filter(u => targetRoles.includes(u.designation));
    }, [users]);
    
    // Auto-fill BRP details from selection
    const watchedBrpCode = form.watch("brpEmployeeCode");
    useEffect(() => {
        if (watchedBrpCode) {
            const brpDetails = users.find(u => u.employeeCode === watchedBrpCode);
            if (brpDetails) {
                                                    ...
                                                    <PopoverTrigger asChild>
                                                        <FormControl>
                                                            <Button variant="outline" role="combobox" className={cn("justify-between", !field.value && "text-muted-foreground")}>
                                                                {field.value
                                                                    ? staffList.find((u) => u.employeeCode === field.value)?.name
                                                                    : "Select Employee..."}
                                                                ...
                                                                <CommandGroup>
                                                                    {staffList.map((u) => (
                                                                        <CommandItem
                                                                            ...
                                                                            key={u.id}
                                                                            onSelect={() => {
                                                                                form.setValue("brpEmployeeCode", u.employeeCode);
                                                                                setBrpEmployeeCodeOpen(false);
                                                                            }}
                                                                        >
